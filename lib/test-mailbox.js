...
function TestMailbox(options) {
  ...
  function onData(stream, session, callback) {
    ...
    var headers = {};
    saveStream.on('headers', function (h) {
      for (var en of h.entries()) {
        if (en[0] === 'from' || en[0] === 'to') {
          headers[en[0]] = en[1].value;
        } else {
          headers[en[0]] = en[1];
        }
      }
    });
    saveStream.on('data', function (data) {
      var mail = {}
      for (var key in data) {
